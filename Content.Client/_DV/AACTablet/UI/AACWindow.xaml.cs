...
using Content.Shared._DV.AACTablet; // starcup
...
public sealed partial class AACWindow : FancyWindow
{
    ...
    [Dependency] private readonly IEntityManager _entMan = default!; // starcup: phrase groups
    private readonly List<QuickPhrasePrototype> _phrases = new();
    ...
    public void Update(EntityUid owner)
    {
        ...
        _phrases.Clear();
        _phrases.AddRange(_prototype.EnumeratePrototypes<QuickPhrasePrototype>().Where(phrase => comp.PhraseGroups.Contains(phrase.PhraseGroup)));
        _phrases.Sort((a, b) => string.CompareOrdinal(a.Group, b.Group));

        PopulateGui();
        FilterSearch(null);
    }
    // end starcup - update method
