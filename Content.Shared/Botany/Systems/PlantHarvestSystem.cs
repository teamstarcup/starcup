...
public sealed partial class PlantHarvestSystem : EntitySystem
{
    ...
    public void DoHarvest(Entity<PlantHolderComponent?> ent, EntityUid user)
    {
        ...
        var totalYield = CalculateTotalYield(plant.Yield, ent.Comp.YieldMod); // DEN - extracted to method
    ...
    /// DEN - extracted yield calculation to its own method
    public static int CalculateTotalYield(int yield, int yieldMod)
    {
        return yield < 0 ? 0 : Math.Max(1, yield * Math.Max(1, yieldMod));
    }
